...
#Pasamos a la creación de tablas. Primero nos encargamos de las 5 que se corresponden con nuestrar entidades del modelo ER
...
Director CHAR(9) NOT NULL,                                #Vemos como el atributo Director como clave foránea establece la relación "DIRIGE" del modelo E-R
...
#En la tabla superior, hemos definido el teléfono de todos los usuarios como NULL por defecto. 
#Sólo dispondremos de esta información en la tabla "USUARIOS" si los usuarios descargan la aplicación.
#Para ello generaremos posteriormente un TRIGGER que regule este proceso.
...
#Pasamos ahora a crear las tablas de relaciones N:N
...
#Finalmente creamos una tabla para el atributo "categoría de la app" por ser multivalorado
...
# Pasamos a crear el TRIGGER que introduzca los números de teléfono de los usuarios que hayan hecho descargas. 
#También creamos otro dos TRIGGER que elimine los teléfonos de los usuarios en el caso de que se borren intancias de la tabla "DESCARGA"
...
#Añadimos ahora las instancias a las tablas de nuestra base de datos. Para ello vamos a usar INSERT INTO directamente
#Lo hacemos así porque será más simple de ejecutar el script para la persona que lo examine.
#Podríamos también cargar el archivo csv con:

/*LOAD DATA INFILE + 'Ruta XXXX.csv'
INTO TABLE + Tabla XXXX
...
#Cabe decir que podríamos haber especificado con NOT NULL aquellos atributos, importantes (pueden ser claves primarias, etc..),
#que no puedan quedar vación en la inserción de datos. No nos ha sido necesario porque nosotros mismos hemos introducido los datos manualmente.
...
#Podríamos crear gran variedad de vistas, TRIGGERS, procesos, consultas, etc... Tantos como fuesen necesarios para gestionar de forma eficiente nuestra base de datos.
#Podemos dejar en este punto la implementación porque consideramos que hemos programado una cantidad y variedad suficientes elementos.
#A partir de este punto y con un poco de esfuerzo e investigación, podemos implementar todo lo que necesitemos y podamos imaginar.
